...
- **Open thread (a live action a finished note carries)** → add the optional `open: >-` field (prefix `NOW` / `NEXT` / `WAITING`). `generate.py` projects every note with a non-empty `open:` into `index/open-questions.md` §Open-threads — so "what's open / what's next" is one grep. **Scope guardrail (load-bearing):** `open:` attaches ONLY to a note that already earns its place as evergreen knowledge (an open decision, an unresolved question, a blocked-on-external dependency). It is **NOT a task tracker** — operational/session tasks live in `MEMORY.md` + session logs. Resolve it → delete the `open:` field; `reconcile` flags an `open:` note untouched >30 days as "resolve or refresh." **Bump `updated:` whenever you add or refresh an `open:`** — the staleness clock keys on the `updated:` date.
