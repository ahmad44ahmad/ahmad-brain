...
def main():
    ...
    # index/open-questions.md — TWO axes in one file (do NOT split into a new index file):
    #   (1) Open threads: a non-empty `open:` field = a finished evergreen note carrying a live
    #       action (NOW/NEXT/WAITING). This is the "what's open" surface (root-cause fix, 2026-06-05).
    #   (2) Draft stubs: status == draft = unfinished notes awaiting expansion.
    # `open:` is ORTHOGONAL to `status` (lifecycle) — never overload status with "open".
    open_notes = [(p, fm) for p, fm in all_notes if str(fm.get("open", "")).strip()]
    open_notes.sort(key=lambda x: x[1].get("id", ""))
    drafts = [(p, fm) for p, fm in all_notes if fm.get("status") == "draft"]
    drafts.sort(key=lambda x: x[1].get("id", ""))
    lines = [f"""---
id: index-open-questions
title: Open Threads & Draft Stubs
type: moc
status: active
...
updated: {date.today().isoformat()}
summary: >-
  Two surfaces in one file: Open threads ({len(open_notes)}) = finished evergreen notes
  carrying a live `open:` action (NOW/NEXT/WAITING); Draft stubs ({len(drafts)}) = notes
  in status draft awaiting expansion. NOT a task tracker — operational tasks live in
  MEMORY.md + session logs.
---

# Open Threads & Draft Stubs

## Open threads ({len(open_notes)})

Finished evergreen notes carrying a live `open:` action item (surfaced from the `open:` frontmatter field; prefix NOW / NEXT / WAITING). **NOT a task tracker** — operational/session tasks belong in `MEMORY.md` + session logs; an `open:` flag attaches only to a note that already earns its place as evergreen knowledge.

"""]
    if open_notes:
        lines.append("| id | title | open (NOW/NEXT/WAITING) |")
        lines.append("|---|---|---|")
        for p, fm in open_notes:
            lines.append(f"| [[{fm.get('id','')}]] | {truncate(fm.get('title',''), 48)} | {truncate(fm.get('open',''), 150)} |")
    else:
        lines.append("_No open threads._")
    lines.append("")
    lines.append(f"## Draft stubs ({len(drafts)})")
    lines.append("")
    lines.append("Notes in `status: draft` — stubs awaiting full migration or Ahmad's next touch.")
    lines.append("")
    if drafts:
        ...
        lines.append("_No draft stubs._")
    (INDEX / "open-questions.md").write_text("\n".join(lines), encoding="utf-8")
    print(f"wrote index/open-questions.md ({len(open_notes)} open threads, {len(drafts)} drafts)")
...
| [[index-entities]] | All `type: entity` — organisations, locations, policies, regulations | {sum(1 for _,fm in wiki_notes if fm.get('type')=='entity')} |
...
| [[index-open-questions]] | **Open threads** (live `open:` items) + draft stubs (`status: draft`) | {sum(1 for _,fm in wiki_notes+decision_notes+log_notes if str(fm.get('open','')).strip())} open / {sum(1 for _,fm in wiki_notes+decision_notes+log_notes if fm.get('status')=='draft')} draft |
