...
Open decisions that the rebuild must NOT silently pick: cross-service interlock semantics (advisory alert vs hard guarded block — existing real procedures are all alert-plus-human-action, so V4 adds hard interlocks like glucose→insulin only as a deliberate improvement) and the re-authored PT clinical assessment form. (The B1-B10 barrier legend is no longer open — it was recovered verbatim; see [[basira-v4-disability-empowerment-model]].)
